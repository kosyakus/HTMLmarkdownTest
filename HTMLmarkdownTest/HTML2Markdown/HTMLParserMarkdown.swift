...
public struct TagMarkdown {
	let name: String
	let attributes: [String: String]
}

public enum ElementMarkdown {
	case root(children: [ElementMarkdown])
	case element(tag: TagMarkdown, children: [ElementMarkdown])
	...
	private func isEmpty(_ children: [ElementMarkdown]) -> Bool {
...
final class HTMLElementMarkdown: Content {
	...
	enum ElementContent {
		...
		private func makeNewElementChild(children: [Content], tokenType: TokenType) throws -> [Content] {
			...
			let newChild = try HTMLElementMarkdown(openingTagStart: tokenType)
...
public struct HTMLParserMarkdown {
	public init() {}
	
	public func parse(html: String) throws -> ElementMarkdown {
		let tokens = try Tokenizer().tokenize(html: html)
		return try HTMLParserMarkdown().parse(tokens: tokens)
	}

	func parse(tokens: [TokenType]) throws -> ElementMarkdown {
		let root = HTMLElementMarkdown()
...
extension Content {
	func result() -> ElementMarkdown {
		if let text = self as? HTMLText {
			return .text(text: text.text)
		}

		if let element = self as? HTMLElementMarkdown {
			...
			case let .closed(tagName, attributes, children):
				let elements = children.map { $0.result() }
				let tag = TagMarkdown(name: tagName, attributes: attributes)
